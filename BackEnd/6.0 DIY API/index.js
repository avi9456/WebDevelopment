...
app.post("/jokes",(req,res)=>{
  ...
  jokes.push(joke);
  // res.status(200).send("new joke add "+JSON.stringify(jokes[jokes.length-1]));
  res.json({"added to jokes ":jokes.slice(-1)});
});
//5. PUT a joke
...
app.delete("/all",(req,res)=>{
  ...
  if(userKey === masterKey){
    ...
    res.status(404).json({error: "your are not authorished to perform this action"});
  }
})
app.listen(port, () => {
